...
class Model():
    ...
    def __init__(self) -> None:
        try:
            ...
            raise error
        self.routine_name = 'print_manager'
        self.event = Event()
        self.thread = None
    ...
    def __start_on_initialize(self):
        ...
        if configuration.start_delay:
            self.event.clear()
            self.thread = Thread(target=print_manager, args=(self.database, self.event, configuration.start_delay), name=self.routine_name)
            self.thread.start()            
    ...
    def start_routine(self):
        ...
        if not self.routine_active():
            self.event.clear()
            self.thread = Thread(target=print_manager, args=(self.database, self.event, ), name=self.routine_name)
            self.thread.start()
